...
- `cache` Cache configuration of agent. The configurable values are the following:
  ...
  - `use_streaming_backend` When enabled Consul client agents will use streaming rpc to
    ...
    have `rpc.enable_streaming` enabled before any client can enable `use_streaming_backend`.
    At least one of `dns.use_cache` or `http_config.use_cache` must be enabled, otherwise
...
- `rpc.enable_streaming` - Enable the gRPC subscribe endpoint on a Consul Server. All
  ...
  `service_health_streaming_backend`. This setting will default to true in a future release of Consul.
